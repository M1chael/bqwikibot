...
describe Wiki do
  ...
  let(:wiki_lib) { double }
  let(:opts) { {minor: false, bot: true, summary: 'Обновление статистики отзывов на 4PDA'} }
  before(:example) do
  ...
  describe '#new' do
    ...
    before(:example) do
      ...
      expect(wiki_lib).to receive(:edit).with('page_name', File.read('test/U_Lite_wiki_rating.txt'), opts)
      @wiki.update(page: 'page_name', type: :reviews, data: [{rating: 7, count: 6}])
		end

    it 'updates count' do
      expect(wiki_lib).to receive(:edit).with('page_name', File.read('test/U_Lite_wiki_reviews.txt'), opts)
      @wiki.update(page: 'page_name', type: :reviews, data: [{rating: 9, count: 7}])      
    end

    it 'updates rating and count' do
      expect(wiki_lib).to receive(:edit).with('page_name', File.read('test/U_Lite_wiki_rating_reviews.txt'), opts)
